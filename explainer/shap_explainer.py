...
class SHAPexplainer:
    ...
    def predict(self, indexed_words):
        ...
        predictions = []
        model_embedding = BertModel.from_pretrained('bert-base-uncased', output_hidden_states = True)
        for sentence in sentences:
            words = ""
            for word in sentence:
                words = words + " " + word
            encoded = self.tokenizer.batch_encode_plus([words], max_length=200, padding='max_length', truncation=True) #apply the tokenization
            encoded = {key:torch.LongTensor(value) for key, value in encoded.items()} #convert tokens in tensors
            with torch.no_grad():
                description = model_embedding(**encoded).last_hidden_state.swapaxes(1,2)#perform the embedding
            
            #description = description.squeeze(0) #get the tensor eliminating a dimension with squeeze
            device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
            description = description.to(device)
            outputs = self.model(description)
